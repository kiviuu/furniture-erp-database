...
CREATE OR ALTER VIEW v_inventory_parts_status AS
SELECT 
    ip.part_id,
    p.part_name,
    ip.available_units,
...
CREATE OR ALTER VIEW v_supplier_offers_detail AS
SELECT 
    so.offer_id,
    p.part_name,
...
CREATE OR ALTER VIEW v_order_parts_status AS
SELECT 
    op.order_part_id,
    p.part_name,
    op.pack_quantity,
...
CREATE OR ALTER VIEW v_delivery_delays AS
SELECT 
    op.order_part_id,
    p.part_name,
    s.company_name AS supplier_name,
...
CREATE OR ALTER VIEW v_supplier_summary AS
SELECT 
    s.company_name,
    ROUND(SUM(so.price * op.pack_quantity), 2) AS total_value,
...
CREATE OR ALTER FUNCTION fn_part_offers_by_deadline (@part_id INT, @deadline DATE)
RETURNS TABLE
AS
RETURN (
    SELECT 
...
CREATE OR ALTER FUNCTION fn_get_supplier_offers_by_name_or_id (
...
CREATE OR ALTER FUNCTION fn_get_late_orders_up_to_date (@deadline_date DATE)
RETURNS TABLE
AS
RETURN (
    SELECT 
...
CREATE OR ALTER VIEW v_client_company_stats AS
SELECT 
    cc.company_name,
    cc.nip,
    COUNT(o.order_id) AS total_orders_count,
    ROUND(SUM(od.quantity * od.unit_price * (1 - o.order_discount)), 2) AS total_orders_value,
    ROUND(AVG(od.quantity * od.unit_price * (1 - o.order_discount)), 2) AS average_order_value,
...
CREATE OR ALTER VIEW v_client_individual_stats AS
SELECT 
    ci.first_name,
    ci.last_name,
    COUNT(o.order_id) AS total_orders_count,
...
CREATE OR ALTER VIEW v_current_and_future_time_offs AS
SELECT 
    e.first_name,
    e.last_name,
    eto.start_date,
...
CREATE OR ALTER VIEW v_client_contact_book AS
SELECT 
    c.client_id,
    CASE 
...
CREATE OR ALTER FUNCTION fn_get_employee_future_time_offs (
...
CREATE OR ALTER VIEW v_product_category_performance AS
WITH PartPrices AS (
    SELECT 
        part_id, 
...
SELECT 
    ...
    ROUND(SUM(cc.line_revenue), 2) AS total_sales_value,
    ...
    ROUND(SUM(cc.line_part_cost), 2) AS total_estimated_parts_cost,
    ROUND(SUM(cc.line_revenue) - SUM(cc.line_part_cost), 2) AS net_income_after_parts
...
CREATE OR ALTER VIEW v_product_sales_summary AS
SELECT 
    p.product_id,
    p.product_name,
    SUM(od.quantity) AS total_units_sold,
    ROUND(SUM(od.quantity * od.unit_price * (1 - o.order_discount)), 2) AS total_sales_value
FROM products p
...
CREATE OR ALTER VIEW v_product_last_order AS
SELECT 
    p.product_id,
    p.product_name,
    MAX(o.order_date) AS last_order_date
...
CREATE OR ALTER VIEW v_inventory_products_status AS
SELECT 
    ip.product_id,
    p.product_name,
    ip.available_units,
...
CREATE OR ALTER VIEW v_product_profitability_analysis AS
...
SELECT 
    p.product_id,
    p.product_name,
    ROUND(p.base_price, 2) AS product_sale_price,
    ROUND(ISNULL(ppc.total_parts_cost, 0), 2) AS total_parts_cost,
    ROUND(p.base_price - ISNULL(ppc.total_parts_cost, 0), 2) AS gross_profit_margin,
    CASE 
        WHEN p.base_price > 0 THEN 
            ROUND(((p.base_price - ISNULL(ppc.total_parts_cost, 0)) / p.base_price) * 100, 2)
        ELSE 0 
...
CREATE OR ALTER FUNCTION fn_get_revenue_report (
    @year INT,
...
RETURN (
    SELECT
        ...
        SUM(od.quantity) AS total_units_sold,
        ROUND(SUM(od.quantity * od.unit_price * (1 - o.order_discount)), 2) AS total_revenue,
...
(
    WITH PartPrices AS (
        SELECT part_id, AVG(price) AS avg_part_price
        FROM supplier_offers
        GROUP BY part_id
    ),
    ProductCosts AS (
        SELECT 
            ...
            p.product_name,
            p.needed_hours,
            ISNULL(SUM(pr.quantity_needed * pp.avg_part_price), 0) AS unit_parts_cost
        FROM products p
        LEFT JOIN product_recipes pr ON p.product_id = pr.product_id
        LEFT JOIN PartPrices pp ON pr.part_id = pp.part_id
        GROUP BY p.product_id, p.category_id, p.product_name, p.needed_hours
    ),
    RawData AS (
        SELECT 
            product_id,
            YEAR(o.order_date) AS r_year,
            CASE 
                WHEN @period_type = 1 THEN CAST(DATEPART(QUARTER, o.order_date) AS VARCHAR)
                WHEN @period_type = 2 THEN CAST(MONTH(o.order_date) AS VARCHAR)
                WHEN @period_type = 3 THEN CAST(DATEPART(WEEK, o.order_date) AS VARCHAR)
                ELSE '0'
            END AS r_period,
            od.quantity AS sold_qty,
            0 AS prod_qty,
            (od.quantity * od.unit_price * (1 - ISNULL(o.order_discount, 0))) AS revenue,
            0 AS calculated_prod_cost,
            0 AS calculated_parts_cost
        ...
        SELECT 
            p.product_id,
            YEAR(ps.start_date) AS r_year,
            CASE 
                WHEN @period_type = 1 THEN CAST(DATEPART(QUARTER, ps.start_date) AS VARCHAR)
                WHEN @period_type = 2 THEN CAST(MONTH(ps.start_date) AS VARCHAR)
                WHEN @period_type = 3 THEN CAST(DATEPART(WEEK, ps.start_date) AS VARCHAR)
                ELSE '0'
            END AS r_period,
            0 AS sold_qty,
            p.quantity AS prod_qty,
            0 AS revenue,
            (p.quantity * (pc.unit_parts_cost + (pc.needed_hours * e.base_salary))) AS calculated_prod_cost,
            (p.quantity * pc.unit_parts_cost) AS calculated_parts_cost
        FROM productions p
        JOIN production_status ps ON p.production_id = ps.production_id
        JOIN employees e ON p.supervisor_id = e.employee_id
        JOIN ProductCosts pc ON p.product_id = pc.product_id
    ),
    AggregatedData AS (
        SELECT 
            product_id,
            ...
            SUM(prod_qty) AS total_produced,
            SUM(revenue) AS total_revenue,
            SUM(calculated_prod_cost) AS total_cost,
            SUM(calculated_parts_cost) AS total_parts_cost
        FROM RawData
        GROUP BY product_id, r_year, r_period
    )
    SELECT 
        CASE WHEN @group_by_category = 1 THEN CAST(pc.category_id AS VARCHAR) ELSE CAST(pc.product_id AS VARCHAR) END AS [ID],
        CASE WHEN @group_by_category = 1 THEN c.name ELSE pc.product_name END AS [Name],
        ad.r_year AS [Year],
        CASE 
            WHEN @period_type = 1 THEN 'Q' + ad.r_period
            WHEN @period_type = 2 THEN DATENAME(month, DATEFROMPARTS(ad.r_year, CAST(ad.r_period AS INT), 1))
            WHEN @period_type = 3 THEN 'Week ' + ad.r_period
            ELSE 'Annual'
        END AS [Period_Label],
        
        SUM(ad.total_sold) AS [Total_Sold_Units],
        SUM(ad.total_produced) AS [Total_Produced_Units],
        CAST(ROUND(SUM(ad.total_produced * pc.needed_hours), 2) AS DECIMAL(18,2)) AS [Total_Produced_Working_Hours],
        CAST(ROUND(SUM(ad.total_parts_cost), 2) AS DECIMAL(18,2)) AS [Total_Parts_Cost],
        CAST(ROUND(SUM(ad.total_revenue), 2) AS DECIMAL(18,2)) AS [Total_Sales],
        CAST(ROUND(SUM(ad.total_cost), 2) AS DECIMAL(18,2)) AS [Total_Production_Costs],
        CAST(ROUND(SUM(ad.total_revenue) - SUM(ad.total_cost), 2) AS DECIMAL(18,2)) AS [Net_Profit]
    FROM AggregatedData ad
    JOIN ProductCosts pc ON ad.product_id = pc.product_id
    JOIN categories_for_product c ON pc.category_id = c.category_id
    GROUP BY 
        CASE WHEN @group_by_category = 1 THEN CAST(pc.category_id AS VARCHAR) ELSE CAST(pc.product_id AS VARCHAR) END,
        CASE WHEN @group_by_category = 1 THEN c.name ELSE pc.product_name END,
        ad.r_year,
        ad.r_period
);
GO
...
(
    SELECT 
        ...
        Total_Produced_Working_Hours,
        Total_Parts_Cost,
        Total_Production_Costs 
    FROM fn_production_cost_profit_report(@group_by_category, @period_type) 
    WHERE Total_Produced_Units > 0
);
GO
...
CREATE OR ALTER FUNCTION fn_get_product_recipe_details (
    @product_id_or_name nvarchar(100)
)
RETURNS TABLE
AS
RETURN
(
    SELECT 
        p.part_id AS [ID],
        p.part_name AS [Nazwa_Czesci],
        pr.quantity_needed AS [Wymagana_Ilosc],
        cp.name AS [Kategoria_Czesci],
        prod.product_name AS [Dla_Produktu]
    FROM products prod
    JOIN product_recipes pr ON prod.product_id = pr.product_id
    JOIN parts p ON pr.part_id = p.part_id
    LEFT JOIN categories_for_part cp ON p.category_id = cp.category_id
    WHERE 
        prod.product_name = @product_id_or_name 
        OR CAST(prod.product_id AS nvarchar(100)) = @product_id_or_name
);
GO


CREATE OR ALTER VIEW v_work_place_status AS
...
CREATE OR ALTER VIEW v_product_production_frequency AS
SELECT 
    p.product_id,
    pr.product_name,
...
CREATE OR ALTER VIEW v_production_delay_details AS
SELECT 
    p.production_id,
    pr.product_name,
    p.quantity,
...
CREATE OR ALTER VIEW v_supervisor_performance_delays AS
SELECT 
    e.employee_id,
    e.first_name + ' ' + e.last_name AS supervisor_name,
...
CREATE OR ALTER VIEW v_work_place_usage_stats AS
SELECT 
    wp.work_place_id,
    wp.work_place_description,
...
CREATE OR ALTER VIEW v_employee_availability_calendar AS
WITH CurrentWork AS (
...
SELECT 
    ...
    CASE 
        WHEN cw.busy_until IS NOT NULL THEN 'Pracuje'
        ELSE 'Dostępny teraz'
...
create or alter view v_production_planned as
select production_id,product_name,quantity,supervisor_id,start_date,planned_end_date from v_production_delay_details
where actual_end_date is NULL and work_place_id is NULL;
go

CREATE OR ALTER FUNCTION fn_get_production_team (
...
CREATE OR ALTER FUNCTION fn_get_employee_production_assignments (
...
create or alter function fn_production_in_progress(
	@start_date DATETIME = '0000-00-00',
	@end_date DATETIME
)
returns table as 
return (
	select production_id,product_name,quantity,start_date,planned_end_date,supervisor_id,delay_days,status_delay from v_production_delay_details where actual_end_date is null and work_place_id is not null
	and start_date>=@start_date and planned_end_date<=@end_date
);
...
CREATE OR ALTER VIEW v_daily_order_stats AS
SELECT 
    CAST(o.order_date AS DATE) AS report_date,
    COUNT(DISTINCT o.order_id) AS total_orders_placed,
    ROUND(SUM(od.quantity * od.unit_price * (1 - o.order_discount)), 2) AS value_placed_today,
    ...
    (SELECT ROUND(SUM(od2.quantity * od2.unit_price * (1 - o2.order_discount)), 2)
     FROM order_details od2
...
CREATE OR ALTER VIEW v_order_financial_summary AS
SELECT 
    o.order_id,
    s.status_name AS current_status,
    ROUND(SUM(od.quantity * od.unit_price * (1 - o.order_discount)), 2) AS total_net_value,
    ROUND(SUM((od.quantity * od.unit_price * (1 - o.order_discount)) * (1 + ISNULL(v.amount, 0))), 2) AS total_gross_value
FROM orders o
JOIN order_details od ON o.order_id = od.order_id
...
CREATE OR ALTER VIEW v_active_delayed_orders AS
SELECT 
    o.order_id,
    o.order_date,
...
CREATE OR ALTER VIEW v_orders_scheduled_today AS
SELECT 
    o.order_id,
    o.client_id,
    s.status_name,
...
CREATE OR ALTER VIEW v_total_orders_scheduled_per_day AS
SELECT 
...
CREATE OR ALTER VIEW v_critical_orders_due_today AS
SELECT 
    o.order_id,
...
CREATE OR ALTER FUNCTION fn_get_client_purchase_history (
...
RETURN (
    SELECT 
        o.client_id, -- Kolumna zachowana
        c.email AS client_email,
        o.order_id,
        o.order_date,
        p.product_name,
        od.quantity,
        CAST(ROUND(od.unit_price, 2) AS DECIMAL(18,2)) AS unit_price,
        o.order_discount,
        CAST(ROUND((od.quantity * od.unit_price * (1 - ISNULL(o.order_discount, 0))), 2) AS DECIMAL(18,2)) AS final_line_value
    FROM orders o
    ...
    WHERE 
        ...
        AND CAST(o.order_date AS DATE) >= @date_from 
        AND o.order_date <= GETDATE()
);
GO

CREATE OR ALTER FUNCTION fn_estimate_order_delivery (@order_id INT)
RETURNS DATETIME
AS
BEGIN
...
CREATE OR ALTER VIEW v_ui_product_categories AS
SELECT 
    category_id, 
...
CREATE OR ALTER VIEW v_ui_part_categories AS
SELECT 
    category_id, 
    name AS category_name
...
CREATE OR ALTER VIEW v_ui_employee_roles AS
SELECT 
    role_id, 
    possition_name, 
    description
...
CREATE OR ALTER VIEW v_ui_production_statuses AS
SELECT 
    status_id, 
    status_name
...
CREATE OR ALTER VIEW v_ui_order_statuses AS
SELECT 
    status_id, 
    status_name
